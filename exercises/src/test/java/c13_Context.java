...
public class c13_Context extends ContextBase {
    ...
    public Mono<Message> messageHandler(String payload) {
        ...
        return Mono.deferContextual(ctx -> {
            String id = ctx.get(HTTP_CORRELATION_ID);
    ...
    public void message_tracker() {
        ...
        Mono<Message> mono = messageHandler("Hello World!").contextWrite(Context.of(HTTP_CORRELATION_ID, "2-j3r9afaf92j-afkaf"));

        StepVerifier.create(mono).expectNextMatches(m -> m.correlationId.equals("2-j3r9afaf92j-afkaf") && m.payload.equals("Hello World!")).verifyComplete();
    }
    ...
    public void execution_counter() {
        ...
        StepVerifier.create(repeat.repeat(4)).thenAwait(Duration.ofSeconds(10)).expectAccessibleContext().assertThat(ctx -> {
            assert ctx.get(AtomicInteger.class).get() == 5;
        }).then().expectComplete().verify();
    }
