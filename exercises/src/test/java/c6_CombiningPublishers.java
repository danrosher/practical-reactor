...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void task_executor() {
        //todo: feel free to change code as you need
        Flux<Void> tasks = taskExecutor().flatMap(i -> i);
    ...
    public void streaming_service() {
        //todo: feel free to change code as you need
        Flux<Message> messageFlux = streamingService().flatMapMany(i -> i);
    ...
    public void mail_box_switcher() {
        ...
        Flux<Message> myMail = mailBoxPrimary().switchOnFirst((signal, flux) ->
                "spam".equals(Objects.requireNonNull(signal.get()).metaData) ? mailBoxSecondary() : flux);
    ...
    public void one_to_n() {
        //todo: feel free to change code as you need
        Flux<String> fileLines = readFile().delaySubscription(openFile());
    ...
    public void acid_durability() {
        ...
        Flux<String> committedTasksIds = tasksToExecute()
                .flatMapSequential(i -> i).doOnNext(this::commitTask);
    ...
    public Mono<String> chooseSource() {
        return Mono.defer(() -> {
            ...
            if (Objects.equals(sourceRef.get(), "B"))
    ...
    public void cleanup() {
        ...
        //StreamingConnection.startStreaming()
        // .flatMapMany(Function.identity()).doOnComplete(() -> StreamingConnection.closeConnection());
