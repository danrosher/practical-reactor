...
public class c5_CreatingSequence {
    ...
    /**
     * Create Callable that increments the counter and returns the counter value, and then use `repeat()`
     * operator to create Flux that emits values from 0 to 10.
     */
    @Test
    public void repeat() {
...
//                Flux.generate(() -> counter,(state,sink) -> {
//            int v = counter.incrementAndGet();
//            sink.next(v);
//            if(v > 9) sink.complete();
//            return state;
    ...
    public void generate_programmatically() {
        ...
        Flux<Integer> generateFlux = Flux.generate(sink -> {
            ...
            if (first.get() >= 5) {
                ...
                sink.next(first.incrementAndGet());
            }
        });
        ...
        Flux<Integer> createFlux = Flux.create(sink -> {
            //todo: fix following code so it emits values from 0 to 5 and then completes
            while(second.get() < 5){
                sink.next(second.incrementAndGet());
            }
            sink.complete();
        });
        ...
        Flux<Integer> pushFlux = Flux.push(sink -> {
            //todo: fix following code so it emits values from 0 to 5 and then completes
            while(third.get() < 5){
                sink.next(third.incrementAndGet());
            }
            sink.complete();
        });
